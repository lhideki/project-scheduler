...
import { toISO } from "../lib/calendar.js";
  ...
  return (
    <div className="h-full overflow-auto p-4">
      <div className="max-w-3xl space-y-8">
        ...
        <div>
          <h3 className="text-sm font-semibold text-slate-700 mb-2">{t("calendar.upcoming.title")}</h3>
          {upcomingHolidays.length === 0 ? (
            <p className="text-xs text-slate-400">{t("calendar.upcoming.empty")}</p>
          ) : (
            <div className="border border-slate-200 rounded-lg divide-y divide-slate-100">
              {upcomingHolidays.map(([date, name]) => (
                <div key={date} className="flex items-center gap-3 px-3 py-1.5 text-xs">
                  <span className="font-mono text-slate-500 min-w-[8rem]">{fmtDate(date)}{t("calendar.weekdaySuffix", { weekday: fmtWeekday(date) })}</span>
