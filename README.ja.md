...
- 矢印キーによるセル移動、セル・行単位のコピー＆ペースト、タスク編集のUndo / Redoに対応しています。
- 「カレンダー編集」タブで、会社独自の休日や休日出勤などの稼働日を指定し、土日・祝日の扱いを上書きできます。
- ガントチャートは日・週・月単位でズームでき、タスクバー内の非稼働日の網掛け、進捗基準日を指定できる稲妻線(進捗線)、バーのホバーやキーボードフォーカスで表示するツールチップに対応しています。
...
- 担当者の週次・月次の稼働上限を考慮した自動リソース平準化に対応しています。工数は担当者の空き容量に応じて早い日から日別に割り当て、上限に達した日を挟んでタスクの期間を延長します。期間内の割当のない日はガントチャートで網掛け表示し、ツールチップで理由を確認できます。
- 「リソース」タブで、担当者ごとの週単位の負荷と稼働上限を確認できます。
- 開始日・終了日・テーマを持つスプリントを定義し、1タスクへ複数のスプリントを割り当てられます。
- スプリント期間と計算後の日程に矛盾がある場合は、アラートで確認できます。
- 依存関係の矛盾(グループを介したものを含む循環参照、依存関係の条件を満たさない開始日、固定マイルストーンの期日超過、削除済みタスクへの依存)を、WBS表とガントチャートで警告します。
- タスクの依存関係をネットワーク図(PERT図)で確認できます。
...
同じプラグインに、保存JSONと[Backlog](https://backlog.com/)のプロジェクトを同期するSkill「backlog-sync」も同梱しています。
Backlog公式CLIの[`bee`](https://github.com/nulab/bee)を使うため、事前に`bee` 1.1以上をインストールして認証してください
(`npm i -g @nulab/bee`の後に`bee auth login`)。

- **Scheduler → Backlog**: 計画から課題を作成・更新します。計算済みの開始日・期限日は`schedule-adjust`の`recalc`で求めるため、JSON形式は変わりません。
- **Backlog → Scheduler**: 課題の状態を`progress`へ取り込み、`schedule-adjust`に引き継いで日程を引き直し、保存します。

同期の設定とタスク・課題の対応表は、保存JSONの隣に置くサイドカーファイル(`<file>.backlog.json`)に保持し、
Project SchedulerのJSONスキーマ自体は変更しません。Backlogへ書き込む前に必ず差分を提示して承認を求め、
課題を自動で削除することはありません。対応表のテンプレートは`.claude/skills/backlog-sync/`を参照してください。
...
- 開発・テストにはNode.js `^20.19.0 || ^22.12.0 || >=24.0.0`(20.19以上の20.x、22.12以上の22.x、または24以上)が必要です。Node.js 21・23は対象外です。これはVitest 4とVite 8の両方が対応する範囲です。GitHub ActionsではNode.js 24を使用しています。
- 同梱のSkill用CLI(`cli.mjs`)自体はNode.js 18以上で動作します。
...
- 稼働日カレンダーの祝日計算は、日本の祝日をもとにした簡易実装です。個別の日付の補正や会社独自の休日は「カレンダー編集」タブで指定してください。
- Undo / Redoの対象はタスクの編集だけです。担当者・スプリント・カレンダーの変更は対象外です。
