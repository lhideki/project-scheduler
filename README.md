...
[Live Demo](https://lhideki.github.io/project-scheduler/) | [Downloadable HTML](project_scheduler.html) | [Feature Overview](https://www.inoue-kobo.com/webservice/project-scheduler-en/) | [Hands-on Tutorial (Japanese)](https://www.inoue-kobo.com/webservice/tutorial-project-scheduler/) | [JSON Format](docs/json-format.md)
...
The Live Demo is published in three variants:

| Page | URL | Interface language |
| --- | --- | --- |
| Default | https://lhideki.github.io/project-scheduler/ | Your saved choice, otherwise your browser's language |
| English | https://lhideki.github.io/project-scheduler/en/ | Always opens in English |
| Japanese | https://lhideki.github.io/project-scheduler/ja/ | Always opens in Japanese |

The English and Japanese pages open in their own language even if you chose another language earlier. If you switch the language from the header on one of those pages, the address changes to the page for the new language, so reloading or bookmarking keeps your choice.

All three pages are served from the same site, so they share the plan saved in your browser. Switching between them shows the same project in a different language.
...
![Version comparison](docs/images/en/compare-version.png)

### Network view

![Network view](docs/images/en/network.png)

### Sprint definition

![Sprint definition](docs/images/en/define-sprint.png)
...
The shared HTML opens in the recipient's language, like the downloadable HTML, even if you exported it from the English or Japanese page of the Live Demo. It does not save changes to `localStorage`. You can still edit the plan after opening it, but reloading the page restores the exported state. To keep your changes, export them as JSON.
...
| `npm run build:readme` | Generates `README.md` from `README.en.md`. |

`npm run build:pages` generates the Live Demo pages from `project_scheduler.html` into `_site/` (`index.html`, `ja/index.html`, and `en/index.html`). It is not part of `npm run build`; GitHub Actions runs it before publishing. Set `PAGES_BASE_URL` to change the public URL used in the `hreflang` links.

### README files

`README.en.md` (English) and `README.ja.md` (Japanese) are edited by hand. `README.md` is generated from `README.en.md` by `npm run build:readme`: it has the same content, except that its Live Demo links point to the Default page. Do not edit `README.md` directly. `npm run test` fails if it is out of date.
...
├── scripts/                 # HTML, Live Demo pages, README, JSON documentation, and Skill CLI generators
